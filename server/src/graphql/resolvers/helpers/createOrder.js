...
const sequelize = require('../../../db/dbConfig'); // we need to access instance for transactions
...
const updatePizzasPrices = require('./updatePizzasPrices');
const { encryptId, decryptId } = require('./cryptId');

sgMail.setApiKey(config.get('sendGridAPI'));

// Get template email paths
...
/**
 * This is a helper function for `createGuestOrder` and `createMemberOrder`
 * mutations.
 * @param {object} attrs
...
async function createOrder(
  ...
  // Wrap in a managed transaction to undo all insertions if something fails
  return await sequelize.transaction(async function (transaction) {
    // create pizzas
    const pizzasDetails = [];
    for (let pizza of pizzas) {
      try {
        const pizzaDetails = await createAndFillPizza(
          pizza,
          context,
          transaction
        );
    ...
    // compute and update pizzas prices; also get overall price and used selection prices and names
    const {
      overallPrice,
      overallQuantity,
      selections,
    } = await updatePizzasPrices(pizzasDetails, context, transaction);
    ...
    let count = 1;
    for (let {
      pizzaRecord: { sauce_id, size_id, crust_id, quantity, price },
      meat_ids,
      veggie_ids,
      cheese_ids,
    } of pizzasDetails) {
